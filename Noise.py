...
class AnalyticNoise(ABC):
    ...
    def __init__(self, frq):
        ...
        # Acceleration noise
        Sa_a = (
            self.DSa_a * (1.0 + (0.4e-3 / frq) ** 2) * (1.0 + (frq / 8e-3) ** 4)
        )  # in acceleration
        self.Sa_d = Sa_a * (2.0 * cp.pi * frq) ** (-4.0)  # in displacement
        Sa_nu = self.Sa_d * (2.0 * cp.pi * frq / CLIGHT) ** 2  # in rel freq unit
        self.Spm = Sa_nu

        # Optical Metrology System
        self.Soms_d = self.DSoms_d * (1.0 + (2.0e-3 / frq) ** 4)  # in displacement
    ...
    def psd(self, option="A", tdi2=False):  # , includewd=0):
        ...
        if option == "X":
            S = (
                16.0
                * cp.sin(x) ** 2
                * (2.0 * (1.0 + cp.cos(x) ** 2) * self.Spm + self.Sop)
            )
        elif option in ["A", "E"]:
            S = (
                8.0
                * cp.sin(x) ** 2
                * (
                    2.0 * self.Spm * (3.0 + 2.0 * cp.cos(x) + cp.cos(2.0 * x))
                    + self.Sop * (2.0 + cp.cos(x))
                )
            )
        elif option == "T":
            S = (
                16.0 * self.Sop * (1.0 - cp.cos(x)) * cp.sin(x) ** 2
                + 128.0 * self.Spm * cp.sin(x) ** 2 * cp.sin(0.5 * x) ** 4
            )
        else:
